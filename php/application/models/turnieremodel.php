...
class TurniereModel
{
	...
	public function get_turnierSpiele($sparte_id, $tu_id){
		$sql = "SELECT spiel.s_id AS SpielID, spiel.ort as Ort, heim.name AS Heim, auswaerts.name AS Auswaerts, spiel.h_tore AS Heimtore, spiel.a_tore AS Auswaertstore, spiel.zeit as Uhrzeit, `status`.`status` AS Status
			FROM spiel
